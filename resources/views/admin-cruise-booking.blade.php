                            ...
                            <ul>
                                ...
                                <li class="submenu">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            <a href="{{url('admin-cruise-booking')}}" class="fs-14 d-inline-flex align-items-center active">Cruise</a>
                                        </li>
                                        <li>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h4 class="mb-0">Cruise Bookings</h4>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover">
                                            ...
                                            <tbody>
                                                @forelse($cruiseBookings as $booking)
                                                <tr>
                                                    <td><a href="#" class="link-primary">#C-{{ $booking->id }}</a></td>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <img src="{{ $booking->bookable ? URL::asset($booking->bookable->image ?? 'build/img/cruise/cruise-01.jpg') : URL::asset('build/img/cruise/cruise-01.jpg') }}" class="rounded me-2" width="40" height="40" alt="Cruise">
                                                            <div>
                                                                <div class="fw-medium">{{ $booking->bookable ? ($booking->bookable->name ?? 'Cruise Package') : 'Cruise Package' }}</div>
                                                                <small class="text-muted">{{ $booking->bookable ? ($booking->bookable->departure_port ?? 'From') : 'From' }} → {{ $booking->bookable ? ($booking->bookable->destination_port ?? 'To') : 'To' }}</small>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>{{ $booking->bookable ? ($booking->bookable->departure_date ?? $booking->created_at->format('Y-m-d')) : $booking->created_at->format('Y-m-d') }}</td>
                                                    <td>{{ $booking->bookable ? ($booking->bookable->duration_days ?? '7') : '7' }} Days</td>
                                                    <td>{{ $booking->bookable ? ($booking->bookable->cabin_type ?? 'Standard') : 'Standard' }}</td>
                                                ...
                                                <tr>
                                                    <td colspan="9" class="text-center py-4">
                                                        <i class="isax isax-ship fs-48 text-muted mb-3"></i>
                                                        <h6 class="text-muted">No Cruise Bookings Found</h6>
                                                        <p class="fs-14 text-muted">Cruise bookings will appear here when customers make reservations</p>
                                                    </td>
                                                </tr>
                                                @endforelse
                                            </tbody>
                                    ...
                                    @if($cruises->count() > 0)
                                    <div class="mt-4">
                                        <h6 class="mb-3">Available Cruise Packages</h6>
                                        <div class="row">
                                            @foreach($cruises->take(6) as $cruise)
                                            <div class="col-md-4 mb-3">
                                                <div class="card">
                                                    <img src="{{ URL::asset($cruise->image ?? 'build/img/cruise/cruise-01.jpg') }}" class="card-img-top" alt="{{ $cruise->name }}" style="height: 150px; object-fit: cover;">
                                                    <div class="card-body p-2">
                                                        <h6 class="card-title mb-1">{{ $cruise->name ?? 'Cruise Package' }}</h6>
                                                        <p class="card-text small text-muted mb-1">{{ $cruise->departure_port ?? 'Port' }} → {{ $cruise->destination_port ?? 'Port' }}</p>
                                                        <p class="card-text small mb-1">${{ number_format($cruise->price ?? 0, 2) }}</p>
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <small class="text-muted">{{ $cruise->bookings->count() }} bookings</small>
                                                            <a href="{{ route('add-cruise') }}" class="btn btn-sm btn-primary">Edit</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
