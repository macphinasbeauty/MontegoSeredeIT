...
(function () {
  ...
  // Datepicker - Only initialize if not already initialized with specific flight search config
  if ($(".datetimepicker").length > 0) {
    $(".datetimepicker").each(function() {
      // Skip flight search date pickers that are already configured
      if ($(this).hasClass('departure-date') || $(this).hasClass('return-date') ||
          $(this).hasClass('multi-departure-date') || $(this).closest('.flight-search-form').length > 0) {
        return;
      }
      // Initialize other date pickers with DD-MM-YYYY format
      $(this).datetimepicker({
        format: "DD-MM-YYYY",
        icons: {
          up: "fas fa-angle-up",
  ...
  if ($(".popular-hotel-slider").length > 0) {
	...
	// Datetimepicker Date - Skip flight search date pickers
	if ($('.datetimepicker').length > 0) {
		$('.datetimepicker').each(function() {
			// Skip flight search date pickers that are already configured
			if ($(this).hasClass('departure-date') || $(this).hasClass('return-date') ||
  ...
  // Coming Soon
	if ($('.comming-soon-pg').length > 0) {
		...
		function setCountdown() {
			...
			// Set countdown date
			let countdownDate = new Date('Jan 22, 2026 16:00:00').getTime();
  ...
  if ($(window).width() <= 767) {
    $('.footer-widget h5').on('click', function() {
      $(this).toggleClass('active');
  ...
  $(".payment-form .form-check-input").on("change", function (e) {
    ...
    if (radioval === "credit-card") {
      ...
      $(".stripe-details").hide();
      $(".mpesa-details").hide();
    } else if (radioval === "stripe") {
      $(".credit-card-details").hide();
      $(".paypal-details").hide();
      $(".stripe-details").show();
      $(".mpesa-details").hide();
    } else if (radioval === "mpesa") {
      $(".credit-card-details").hide();
      $(".paypal-details").hide();
      $(".stripe-details").hide();
      $(".mpesa-details").show();
